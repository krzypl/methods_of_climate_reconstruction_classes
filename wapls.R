...
# 
# write_csv(fos_wapls, "data/example_fos_correct_names.csv")

harmonized_names <- read_csv("data/harmonized_names.csv")

fos_wapls <- read_csv("data/example_fos_correct_names.csv")
...
#predict the core
pred <- predict(fit, fos_wapls, sse = TRUE, nboot = 1000)

 reconPlot(pred$fit.boot[, 2],
          depth = age,
          rev.x = TRUE,
          errors = pred$SEP.boot[, 2],
          use.labels = TRUE,
          display.error = "bars",
          xlab = "Wiek (lata BP)",
          ylab = "Temperature lata (st. C)")
